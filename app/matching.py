...
def match_job_ids(
    ...
    # 地点可能填多个城市（逗号/顿号/分号/斜杠/空格混用），拆开做"任一命中"，
    # 否则整串当一个地点会匹配不到任何职位。
    locations = [
        x.strip().lower()
        for x in re.split(r"[,\uFF0C\u3001;\uFF1B/\s]+", sub.location or "")
        if x.strip()
    ]
    ...
    with session_scope() as session:
        ...
        matched: list[int] = []
        seen_keys: set[tuple] = set()  # 同一职位可能有历史重复行，按(站点,标题,公司)去重
        for job in jobs:
            ...
            # 关键词只在“标题 + 技能标签”里匹配：不扫 JD 正文与公司名。
            # 否则像“测试”这种词几乎每条开发岗 JD 都提到（单元测试/测试用例），
            # 会把大量“XX开发”岗误召回；标题/标签才真正代表岗位方向。
            kw_hay = " ".join(x.lower() for x in (job.title, job.tags) if x)
            if not _keywords_ok(kw_hay, sub.keywords):
                continue
            # 工作性质（全职/实习/远程等）仍可参考更全的文本
            nature_hay = " ".join(
                x.lower() for x in (job.title, job.tags, job.description) if x
            )
            if not _job_type_ok(nature_hay, job_type):
                continue
            if locations:
                jl = (job.location or "").lower()
                if jl and not any(loc in jl for loc in locations):
                    continue
