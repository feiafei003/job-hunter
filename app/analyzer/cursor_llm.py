...
def _cursor_proxy_candidates() -> list[str]:
    """Cursor 出网代理候选：优先用 cursor_proxy_url，再并入浏览器代理轮转列表。"""
    cands: list[str] = []
    cur = (_settings.cursor_proxy_url or "").strip()
    if cur:
        cands.append(cur)
    for p in _settings.browser_proxy_list:
        if p and p not in cands:
            cands.append(p)
    return cands


def _pick_cursor_proxy() -> str:
    """挑一个当前连得通的代理；都不通则退回首选。不走代理时返回 ""。"""
    if not _settings.cursor_use_proxy:
        return ""
    cands = _cursor_proxy_candidates()
    if not cands:
        return ""
    try:
        from ..browser import pick_working_proxy

        return pick_working_proxy(cands)
    except Exception:  # noqa: BLE001
        return cands[0]


def _run_via_relay(prompt: str, proxy: "str | None" = None) -> str:
    ...
    proxy = (proxy or _settings.cursor_proxy_url or "").strip() or None
...
def _run_prompt(prompt: str, proxy: "str | None" = None) -> str:
    """在独立子进程里调用 Cursor SDK，规避 Windows asyncio 线程问题。

    若配置了 cursor_relay_url，则改走中转服务器（本机连不上 Cursor 云端时）。
    proxy：本次调用使用的出网代理（由上层挑选的可用代理）；None 则回退到配置值。
    """
    if _relay_enabled():
        return _run_via_relay(prompt, proxy)
    ...
    if _settings.cursor_use_proxy:
        ...
        proxy_url = (proxy or _settings.cursor_proxy_url or "").strip()
        if proxy_url:
...
_RETRYABLE_HINTS = (
    "internal", "500", "timeout", "timed out", "unavailable", "503", "502",
    # 代理抽风/连接类，可换代理重试
    "proxy", "connect", "reset", "464", "无输出", "no output", "eof", "broken pipe",
)
_MAX_ATTEMPTS = 3
...
async def _run_with_retry(prompt: str) -> str:
    ...
    for attempt in range(1, _MAX_ATTEMPTS + 1):
        proxy = await asyncio.to_thread(_pick_cursor_proxy)  # 每次重试都挑当前可用代理
        try:
            return await asyncio.to_thread(_run_prompt, prompt, proxy)
...
async def analyze_job(job: dict, profile: str | None = None) -> Dict[str, Any]:
    ...
    for attempt in range(1, _MAX_ATTEMPTS + 1):
        proxy = await asyncio.to_thread(_pick_cursor_proxy)  # 每次重试都挑当前可用代理
        try:
            content = await asyncio.to_thread(_run_prompt, prompt, proxy)
