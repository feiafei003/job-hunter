...
# 页面 HTML 不做强缓存：每次revalidate（配合 FileResponse 的 ETag/Last-Modified，
# 未改动时返回 304 很轻量），避免更新后用户看到旧页面。
...
def index():
    ...
    return FileResponse(WEB_DIR / "portal.html", headers=_NO_CACHE)
...
def subscribe_page(token: str = ""):
    ...
    return FileResponse(WEB_DIR / "subscribe.html", headers=_NO_CACHE)
...
def browser_view_page():
    ...
    if _app_role() == "user":
        return JSONResponse({"detail": "Not found"}, status_code=404)
    return FileResponse(WEB_DIR / "browser.html", headers=_NO_CACHE)
